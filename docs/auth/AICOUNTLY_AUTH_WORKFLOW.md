# AICOUNTLY auth workflow (Advisor)

How Advisor signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `advisor.aicountly.com` (or `advisor.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/advisor?returnUrl={origin}/auth/callback`.
...
| Advisor host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `advisor.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `advisor.aicountly.com/api` |
| `advisor.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `advisor.gh.aicountly.com/api` |
...
`advisor` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://advisor.gh.aicountly.com/api/health
...
curl -i -X POST https://advisor.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://advisor.gh.aicountly.com/api/global/login   # expect 404
```
